...
<StackLayout @loaded="onLoaded_Rendering(0, 250)">
    <PuasaAccordion v-if="!rendering0" v-for="(item, index) in items" :key="index" :index="index" :judul="item.judul">
        <StackLayout slot="item">
            <Gradient direction="top down" colors="#28ADAA, #28BAAA">
                <StackLayout padding="0 0 10 0">
                  <PuasaItemTextbox title="Keterangan" :description="item.keterangan" />
                </StackLayout>
            </Gradient>

            <StackLayout padding="10 0" class="left-right-border">
                <PuasaItemCheckboxReadonly v-for="(_item, _index) in item.payload_description" :key="_index" :description="_item.description" :checked="_item.selected"/>
            </StackLayout>
