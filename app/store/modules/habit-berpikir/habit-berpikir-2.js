...
const getters = {
	[get_habit_berpikir_kedua_payload]:(state) => {
		return state.payload;
	},
  [get_habit_berpikir_kedua_payload_length]:(state) => (data) => {
		return state.payload[data].payload_description.length;
	},
  [get_habit_berpikir_kedua_payload_checked]:(state) => (data) => {
    const { payload } = state;
...
const mutations = {
	[set_habit_berpikir_kedua_payload]:(state, data) => {
		state.payload = data
	},
  [set_habit_berpikir_kedua_payload_checked]:(state, data) => {
...
const actions = {
	[action_habit_berpikir_kedua_payload]: ({commit}, payload) => {
			commit('set_habit_berpikir_kedua_payload', payload);
	},
};
