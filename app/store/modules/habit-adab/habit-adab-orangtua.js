...
const JSON = require("@/data/adab-orangtua.js");
...
const getters = {
	[get_habit_adab_orangtua_payload]:(state) => {
		return state.payload;
	},
  [get_habit_adab_orangtua_payload_length]:(state) => (data) => {
		return state.payload[data].payload_description.length;
	},
  [get_habit_adab_orangtua_payload_checked]:(state) => (data) => {
    const { payload } = state;
...
const mutations = {
	[set_habit_adab_orangtua_payload]:(state) => {
		state.payload = data
	},
  [set_habit_adab_orangtua_payload_checked]:(state, data) => {
...
const actions = {
	[action_habit_adab_orangtua_payload]: ({commit}, payload) => {
			commit('set_habit_adab_orangtua_payload', payload);
	},
};
