...
const JSON = require("@/data/habit-adab/adab-guru");
...
const getters = {
	[get_habit_adab_guru_payload]:(state) => {
		return state.payload;
	},
  [get_habit_adab_guru_payload_length]:(state) => (data) => {
		return state.payload[data].payload_description.length;
	},
  [get_habit_adab_guru_payload_checked]:(state) => (data) => {
    const { payload } = state;
...
const actions = {
	[action_habit_adab_guru_payload]: ({commit}, payload) => {
			commit('set_habit_adab_guru_payload', payload);
	},
};
