...
const JSON = require("@/data/habit-zis/habit-zis-waqaf");
// console.log(zis_waqafJSON);
...
const getters = {
	[get_habit_zis_waqaf_payload]:(state) => {
...
const mutations = {
  ...
  [set_habit_zis_waqaf_payload_edit]:(state, data) => {

    // "id":"1",
		// "code":"729ca0eb-2422-388e-95bb-120ffea771e1",
		// "initial": "HABIT-zis_waqaf",
...
const actions = {
	[action_habit_zis_waqaf_payload]: ({commit}, payload) => {
			commit('action_habit_zis_waqaf_payload', payload);
	},
  [action_habit_zis_waqaf_payload_add]: ({commit}, payload) => {
			commit('action_habit_zis_waqaf_payload_add', payload);
	},
  [action_habit_zis_waqaf_payload_remove]: ({commit}, payload) => {
      commit('action_habit_zis_waqaf_payload_remove', payload);
  },
  [action_habit_zis_waqaf_payload_edit]: ({commit}, payload) => {
      commit('action_habit_zis_waqaf_payload_edit', payload);
  },
};
